...
**Passive-Embed-Syntax (`+[[…]]`).** Reines Pre-Processing, kein eigener AST-Pfad: vor `pandoc.read` wird `+[[` durch `![[` ersetzt (Lua-Pattern `%+%[%[`) — danach ist die Note in jeder Hinsicht ein normaler Image-Inline-Embed und durchläuft denselben Code wie `![[…]]` (Wraps, Auto-Heading-Shift, autoref-Registrierung, Slices, Image-Figures, Mermaid). Zwei Pre-Processing-Stellen: (i) `build.sh` macht den Rewrite auf der Top-Level-Datei via `sed 's/+\[\[/![[/g'`, weil Pandoc die Top-Level direkt parst und den Lua-Filter erst auf dem AST sieht; (ii) `obsidian-transclude.lua`'s `load_note` macht denselben Rewrite via `content:gsub` nach dem `io.open` und vor `pandoc.read`, damit die Syntax auch in rekursiv eingebundenen Notes greift. **Beide Stellen müssen synchron bleiben** — wer eine ändert (z.B. Boundary-Constraint hinzufügt), muss die andere mitziehen. Top-Level-Datei wird nach `$WORK/$BASE.md` kopiert (Vault ist read-only), Pandoc liest dann `$PROCESSED_INPUT` statt `$INPUT_ABS`. Resource-Path bleibt unverändert (zeigt auf das Original-Verzeichnis im Vault), Lua-`find_note_path` walked `PANDOC_STATE.resource_path`, also funktionieren Embed-Lookups weiterhin. Semantik bewusst syntax-äquivalent — der einzige Unterschied ist Obsidians Render-Verhalten in der Editor-/Reader-Ansicht (passiv = Obsidian rendert NICHT als Embed, bleibt als Plus-präfixierter Wikilink-Text sichtbar; Pipeline expandiert wie gewohnt).
...
**2. Zitationen.** v1 implementiert nach Branding-Override-Pattern. TS-Seite (`src/utils/bibliography.ts` → `prepareBibliography`) liest `bibliography:` und optional `csl:` aus dem Doc-Frontmatter via `metadataCache.getFileCache`, resolved den linkpath via `metadataCache.getFirstLinkpathDest` (akzeptiert Plain-Pfad `"References/Sources.bib"` und Wikilink `"[[Sources.bib]]"` — `stripWikilink`-Helper entfernt Brackets, Alias und Hash), und kopiert die Files unter Fixnamen nach `$WORK/`: `references.bib` und `citation-style.csl`. `build.sh` macht nur noch `-f`-Probe auf die zwei Pfade und hängt `--citeproc --bibliography=… [--csl=…]` als `EXTRA_BIB_ARGS` an die Pandoc-Invocation — kein YAML-Parsing, kein Vault-Find, keine Bash-Quoting-Probleme mit Spaces in Vault-Pfaden (`References /Obsi Test.bib` mit trailing Space im Folder-Name wird sauber resolved, weil Obsidians Index basename-indiziert). CSL-Resolution zwei-stufig in TS: erst `<pluginDir>/pipeline/app/csl/<name>.csl` (curated preinstalled, aktuell leeres Verzeichnis — Pandoc-Default greift dann), dann Vault-Linkpath. `cslPreinstalled`-Flag im Result fließt nur in die Notice. Aufruf in `export.ts` zwischen `prepareBrandingOverride` und `runPipeline`; Notice bei Erfolg, Abort + Notice bei Resolve-Fehler. Cleanup in `build.sh` räumt beide Files nach erfolgreichem Build mit weg (`rm -f $WORK/references.bib $WORK/citation-style.csl`), kein State-Carryover zwischen Exports. **Filter-Order-Stolperstein:** `${EXTRA_BIB_ARGS[@]}` (= `--citeproc --bibliography=…`) muss in der Pandoc-Invocation **NACH** den `--lua-filter`-Flags stehen. Pandoc 2.11+ führt Filter und Citeproc in der Reihenfolge der Kommandozeile aus; steht `--citeproc` davor, sieht es nur Top-Level-Citations — Citations innerhalb von `![[…]]`-Embeds werden erst von `obsidian-transclude.lua` expandiert und entgehen Citeproc komplett. Symptom: Citation-Marker erscheinen als escaped Roh-Text (`{[}@key{]}`) im `.tex` statt als formatierte Citation (`(Vaswani u. a. 2023)`). Direkt-Export der citation-haltigen Note funktioniert trotzdem (Citations sind dort Top-Level), Embed-Export der gleichen Note via Hauptdokument schlägt fehl — der trügerische Fall, der die Diagnose schwer machte. User-Syntax (Standard-Pandoc): `[@key]`, `[-@key]` (Author-Suppress), `@key` (Author-in-Text), `[@key, p. 42]` (Locator), `[@a; @b]` (Multi). Bibliography-Placeholder `::: {#refs}\n:::` für expliziten Platz, ohne hängt Pandoc das Verzeichnis automatisch ans Doc-Ende. Test: `ExampleFiles/HelperFiles/cite-test.md` gegen `References /Obsi Test.bib`. Empfohlener Authoring-Workflow für User (im Skill dokumentiert, kein Plugin-Code nötig): Zotero + Better-BibTeX-Plugin (auto-export `.bib` in den Vault) + Obsidian Citations- oder Zotero-Integration-Plugin (Quick-Insert). Offen für v2: CSL-Stile preinstallieren (`/app/csl/`-Verzeichnis mit APA, Chicago, IEEE, MLA, DIN-1505-2); Doc-lokale Bibliography über `references:`-CSL-JSON-Liste im Frontmatter als Eskalations-Layer ohne `.bib`-File (Pandoc-citeproc nativ).
